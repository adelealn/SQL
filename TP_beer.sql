...
#	(SELECT SUM(QUANTITE) FROM ventes INNER JOIN article USING (ID_ARTICLE) INNER JOIN type USING (ID_TYPE) WHERE ANNEE = 2015 AND NOM_TYPE=TYPE_BIERE) AS VENTES_2015,
#	(SELECT SUM(QUANTITE) FROM ventes INNER JOIN article USING (ID_ARTICLE) INNER JOIN type USING (ID_TYPE) WHERE ANNEE = 2016 AND NOM_TYPE=TYPE_BIERE) AS VENTES_2016,
#    (SELECT VENTES_2016-VENTES_2015) AS EVOLUTION_VENTES
#FROM type
#ORDER BY EVOLUTION_VENTES DESC;
#Q30
#SELECT * FROM ticket WHERE ticket.NUMERO_TICKET NOT IN (SELECT ventes.NUMERO_TICKET FROM ventes) AND ticket.ANNEE NOT IN (SELECT ventes.ANNEE FROM ventes);
#Q31
#SELECT ID_ARTICLE, NOM_ARTICLE,
#	(SELECT SUM(QUANTITE) FROM ventes WHERE ANNEE=2016 AND ventes.ID_ARTICLE = article.ID_ARTICLE) AS QUANTITE_VENDUE_2016,
#	(SELECT SUM(QUANTITE) AS SOMME_QUANTITE FROM ventes WHERE ANNEE=2016 GROUP BY ID_ARTICLE ORDER BY SOMME_QUANTITE DESC LIMIT 1) AS QUANTITE_MAX_VENDUE_2016
# FROM article WHERE ID_ARTICLE IN (SELECT ID_ARTICLE FROM ventes WHERE ANNEE=2016)
# HAVING QUANTITE_VENDUE_2016 >= ROUND(QUANTITE_MAX_VENDUE_2016 - QUANTITE_MAX_VENDUE_2016*0.15);
#Q32
#UPDATE article SET PRIX_ACHAT=PRIX_ACHAT+PRIX_ACHAT*0.1 
...
SELECT NOM_ARTICLE, TITRAGE FROM article 
	INNER JOIN ( SELECT MIN(TITRAGE), ID_COULEUR, ID_TYPE FROM article GROUP BY ID_COULEUR, ID_TYPE ) AS MINIMUM
    ON article.ID_TYPE = MINIMUM.ID_TYPE AND article.ID_COULEUR =MINIMUM.ID_COULEUR
